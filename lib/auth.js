...
const adminbox = require("../parts/adminbox.js");
...
module.exports = {
    ...
    statusScreenBtn : function(req,res){
        let statusScreenBtn = '<div class="sm_btn_box"><button><a href="/account/login">Login</a></button></div>';
        if(this.isUser(req,res)){ 
            statusScreenBtn = `<div class="sm_btn_box"><button class="margin_right_s"><a href="/account/info/page">MyPage</a></button><button><a href="/account/logout">Logout</a></button></div>`
        }
    ...
    statusAdminBtn: function(req,res){
        let statusAdminBtn = '';
        if(this.isUser(req,res)){
            if(req.session.code === init.admin.code){
                statusAdminBtn = `<div class="flex_col"><button class="margin_bottom"><a href="/admin/member">Admin_Member</a></button><button><a href="/admin/board">Admin_Board</a></button></div>`;
            }
        }
        return statusAdminBtn;
    },
    statusAdminPanel:function(req,res){
        let statusAdminPanel = '';
        if(this.isUser(req,res)){
            if(req.session.code === init.admin.code){
                statusAdminPanel = `
                    ${adminbox}
