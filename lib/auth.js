        ...
        if(this.isUser(req,res)){           // login O
            if(data.mcode != 0){                // guest post X 
                if(req.session.code == data.mcode)  // author == user => UI O
                    authStatusReadBtn = `<button><a href="/board/new">write</a></button> <button><a href="/board/review/${data.pcode}" onclick="return _UPDATE_POST(${data.pcode});">update</a></button> <button><a href="/board/list/${data.pcode}" onclick="return _DELETE_POST(${data.pcode});">delete</a></button> <button><a href="/board/${req.params.boardId}/${req.params.pageId}">list</a></button>`;
            }
        }else{                              // login X
